...
@Test func expressionUpdateQueuesAnUpload() async throws {
    ...
    let remote = InMemorySyncRemote()
    let engine = try SyncEngine.forTesting(db: db, remote: remote, tables: [SyncTable(name: "counters")])
...
@Test func batchStatementQueuesOneEntryPerRow() async throws {
    let db = try recipesDB()
    _ = try SyncEngine.forTesting(db: db, remote: InMemorySyncRemote(), tables: [SyncTable(name: "recipes")])
...
@Test func multiTableTransactionCommitsRowsAndOutboxTogether() async throws {
    ...
    _ = try SyncEngine.forTesting(db: db, remote: InMemorySyncRemote(), tables: recipeGraphTables)
...
@Test func initThrowsWhenADeclaredTableIsMissingLocally() async throws {
    ...
    #expect(throws: SyncError.self) {
        _ = try SyncEngine.forTesting(db: db, remote: InMemorySyncRemote(), tables: [SyncTable(name: "ghost")])
    }
}
...
@Test func initThrowsWhenThePrimaryKeyColumnIsMissing() async throws {
    let db = try recipesDB()
    #expect(throws: SyncError.self) {
        _ = try SyncEngine.forTesting(db: db, remote: InMemorySyncRemote(), tables: [SyncTable(name: "recipes", primaryKey: "uuid")])
    }
}
...
@Test func reinstallingTriggersIsIdempotent() async throws {
    ...
    let db = try recipesDB()
    let first = try SyncEngine.forTesting(db: db, remote: InMemorySyncRemote(), tables: [SyncTable(name: "recipes")])
    let second = try SyncEngine.forTesting(db: db, remote: InMemorySyncRemote(), tables: [SyncTable(name: "recipes")])
