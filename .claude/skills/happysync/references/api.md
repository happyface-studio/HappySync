...
| `func stop() async` | **Async** — awaits the in-flight pass *and any in-flight `enqueue` transaction*, then quiesces (no more DB writes / network) and unsubscribes Realtime. Settles `status` to `.idle` but leaves subscriptions open. `start()` re-subscribes cleanly. Call **before** wiping/replacing the DB. |
| `func enqueue(_ op: SyncOp, table: String, row: some Encodable & Sendable) async throws` | Writes the domain row + an outbox entry in one transaction and wakes the runner. `.delete` cascades to FK children. Throws `SyncError.unknownTable` / `.missingPrimaryKey` / `.encoding`. |
