...
Running a synced app: what the status means, how a failed write is classified, how to repair one the
server refused, and what to do when a symptom shows up in the field.
...
- `failedUploads > 0` → some local changes haven't reached the server yet (degraded, still
  retrying with backoff).
- `deadLetters > 0` → some writes parked and need inspection/repair.
...
- **Transient** (network, 5xx, 408/429, transient Postgres `40001`/`40P01`/`53xxx`/`08xxx`, unknown
  codes) → retried with per-entry exponential backoff (jittered) until a cap, then dead-lettered.
- **Auth-shaped** (401/403, `PGRST301`/`PGRST302`) → transient **and exempt from the retry budget**,
  so an expired-token stretch never dead-letters the whole outbox; it recovers when the token
...
try await engine.retryDeadLetters()            // all, or pass [seq] for specific entries

// Or abandon the local write and accept the server's version — discard drops the entries and
// re-pulls the affected rows so local converges to the server (including changes missed while parked):
...
```swift
await engine.stop()          // quiesced here
...
never saw may already be purged. Once `now − lastSyncedAt > maxOfflineGap` the engine clears
cursors, re-pulls, and reconciles purged deletes instead of resurrecting them. Keep `maxOfflineGap`
**≤ the server's tombstone-purge retention** (see <doc:ServerSetup>). For a `scopeColumn` table the
resync is deferred until a partition value resolves, so it never wipes an un-pulled table.
...
- **Never rename in place.** Add the new name, dual-write server-side, wait for adoption, then
  remove the old name by the removal rule.
- **Adding a column — server-first.** Deploy it (nullable/default) before any client reads/writes
  it. Old clients drop the unknown column on download and don't send it on upload.
...
| Deleted parent leaves orphaned children in the UI | Child FK not declared `ON DELETE CASCADE` locally, or missing server child-tombstone trigger | Declare `ON DELETE CASCADE` on the child FKs (`dependsOn` alone deletes nothing) and add the server child-tombstone trigger. |
| `SyncEngine.init` throws `invalidManifest` | A declared `SyncTable` disagrees with the local schema — a table or column that isn't there, a duplicate declaration, `dependsOn` naming an undeclared table, `conflictColumns` on a table something foreign-keys, or `serverColumns` without the primary key | Read the message: it names the table, the field and the value. Run your app's migrations **before** constructing the engine, then fix the manifest. It throws rather than silently syncing the wrong thing — every one of these used to be invisible until the damage showed up on another device. |
